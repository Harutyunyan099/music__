#!/usr/bin/env python3
"""muzzz — local server and YouTube Data API proxy.

Run:      python3 server.py            (default port 8000)
Open:     http://localhost:8000

The YouTube API key is read from the environment or from a local .env file and
never leaves this process: the browser only ever talks to /api/* on this server.

    YOUTUBE_API_KEY=AIza...            required for search
    PORT=8000                          optional
    YOUTUBE_API_BASE=https://...       optional, for testing against a mock
"""
...
class UpstreamError(Exception):
    def __init__(self, code, reason):
        ...
        self.code = code          # status sent back to the browser
...
def shape_items(search_payload):
    """search.list + videos.list -> flat, frontend friendly track objects."""
    ...
    missing = [i for i in ids if not base[i]['duration']]
    if missing:
        try:
    ...
    tracks = [base[i] for i in ids if base[i].get('embeddable', True)]
    return {
        'items': tracks,
        'nextPageToken': search_payload.get('nextPageToken', ''),
        'totalResults': (search_payload.get('pageInfo') or {}).get('totalResults', len(tracks))
    }
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def handle_api(self, path, params):
        if path == '/api/status':
            self.send_json({'youtube': bool(api_key()), 'version': 2})
            return
    ...
    def serve_range(self, range_header):
        """Partial content so seeking inside local audio works everywhere."""
